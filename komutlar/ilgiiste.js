...
if (!kullanıcı) return message.inlineReply(`Bir Kullanıcı Belirt.`);

let member = message.guild.member(kullanıcı);
    
    if (kullanıcı.id == message.author.id) return message.inlineReply(new Discord.MessageEmbed().setDescription(`Dostum sana üzücü bir haberim var. K-kendine ilgi veremezsin...`).setColor("RED"));
...
let mesaj = await message.inlineReply(böhöhöyt)
await mesaj.react("✅")
await mesaj.react("❌")
...
errors: ['time']
  ...
  .then(message.inlineReply(`Pişt ${kullanıcı}, bi bakar mısın?`)) //mesaj embedli atıldığı için etiketlediğiniz kişiye haber verilsin diye burada etiket atıyor. eğer etiket atmasını istemiyorsanız bu satırı silebilirsiniz
  .then(collected => {
...
message.inlineReply(bunumeftunyapmis)//meftun!#3544
} else {
...
message.inlineReply(bunudameftunyapmis)//meftun!#3544
}
})
}
