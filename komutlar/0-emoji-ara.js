...
exports.run = async (client, message, args) => {
    const error = (str) => message.inlineReply(new Discord.MessageEmbed().setColor('RED').setTitle('Hata').setDescription(str));
    ...
    message.inlineReply(new Discord.MessageEmbed().setTitle('Onaylıyor musunuz?').setColor('GREEN').setDescription(`**${emojis.size} adet** emoji indirilip, ZIP dosyası olarak verilecek.`)).then(m => {
        m.react('✅');
        m.react('❌');
        const emojies = ['✅', '❌'];
                    ...
                    emojis.forEach(async emoji => {
                        const timeout = setTimeout(async () => {
                            ...
                            if (kalan == 0) {
                                ...
                                file.addLocalFolder('./emojilerr', 'Dark Place Emoji Sistemi');
                                fs.writeFileSync('/darkplacebot.zip', file.toBuffer());
                                ...
                                m.edit(embed);
                                message.inlineReply({
                                    files: [{ name: 'darkplace.zip', attachment: '/darkplacebot.zip' }]
                                }).then(() => {
                                    fs.promises.rmdir('emojilerr', { recursive: true }).then(() => console.log("emojiler klasorunu sildim"))
                                });
                            }
                        }, yuklenen * 2000);
                        yuklenen = yuklenen + 1;
...
exports.help = {
    name: 'emoji-ara',
    namee: 'search',
    description: 'emoji arama sistemi, laura tarafından yapıldı.'
};
