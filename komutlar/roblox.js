...
exports.run = async (client, message, args) => {
    ...
    if (!ulke) return message.inlineReply('**Bir Oyuncu Adı Girmelisin!**')



    if (ulke.includes('ç' || 'Ç' || 'ğ' || 'Ğ' || 'İ' || 'ö' || 'Ö' || 'ş' || 'Ş' || 'ü' || 'Ü' || 'ı')) {
        return message.inlineReply('**Oyuncu Adları Türkçe Karakterler İçermemelidir!**')
    }
    ...
    fetch(`http://api.roblox.com/users/get-by-username?username=${ulke}`)
        .then(res => res.json()).then(body => {
            ...
            if (body.success === false) return message.inlineReply('**Böyle Bir Oyuncu Bulunamadı!**')



            if (ulke.length < 3) return message.inlineReply(`**Oyuncu Adları En Az 3 Karakterden Oluşmalıdır!**`)
