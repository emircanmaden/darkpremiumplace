...
exports.run = async (client, message, args) => {
  ...
  if(!args[0]) return message.inlineReply("Bir mesaj ID'si yazman gerek. Daha fazla bilgi için: https://support.discord.com/hc/tr/articles/206346498");
  if(isNaN(args[0])) return message.inlineReply("Bir mesaj ID'si yazman gerek. Daha fazla bilgi için: https://support.discord.com/hc/tr/articles/206346498");
  
   try {
     var m = await message.channel.messages.fetch(args[0]);
   } catch(error) {
     return message.inlineReply("Mesajı bulamadım. Lütfen verdiğiniz mesaj ID'sinin bu kanaldaki bir mesaja ait olduğuna emin olun.");
   };
  ...
  if(emojis.length == 1) return message.inlineReply(new Discord.MessageEmbed()
