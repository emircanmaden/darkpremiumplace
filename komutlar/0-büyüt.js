...
var emoji;
if(s) {
if(!message.guild.emojis.cache.get(s.split(':')[2].split('>')[0])) return message.inlineReply('Emojiyi büyütürken bir hata ile karşılaştım. Lütfen bir emoji attığından emin ol.')
emoji = message.guild.emojis.cache.get(s.split(':')[2].split('>')[0]);
} else {
if(!message.guild.emojis.cache.find(x => x.name === args.slice(0).join('-'))) return message.inlineReply('Emojiyi büyütürken bir hata ile karşılaştım. Lütfen bir emoji attığından emin ol.')
...
return message.inlineReply('Emojiyi büyütürken bir hata ile karşılaştım. Lütfen bir emoji attığından emin ol.');
}
}; 
exports.conf = {
  enabled: true,
