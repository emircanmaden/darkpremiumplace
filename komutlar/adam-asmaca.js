  ...
  let kelime = randomkelime({ exactly: 1 })
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                ...
                if (!guess.size) {
                    await message.inlineReply('Zamanın doldu!');
                    break;
                }
            ...
            if (cevap.length === confirmation.length || tahmin) return message.inlineReply(`**Tebrikler kelimeyi buldun! **${cevap}!`);
            return message.inlineReply(`Maalesef bilemedin kelime bu: **${cevap}**`);
        } catch (err) {
            oyndurum.delete(message.channel.id);
