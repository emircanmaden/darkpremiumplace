...
exports.run = async(client, message, args) => {
 if(talkedRecently.has(message.author.id)) return message.inlineReply(`Bu komutu 10 dakikada bir kullanabilirsin`)
if(!args[0]) return message.inlineReply(`bir şehir adı girin. *Küçük harfler ile*`)
axios.get(`https://api.collectapi.com/pray/all?data.city=${args[0].toLowerCase()}`, {
 headers: {
  ...
  message.inlineReply(new discord.MessageEmbed().setColor("YELLOW").setDescription(`Bir hata oldu`))
})
