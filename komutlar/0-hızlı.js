...
if(!args[0] && !message.attachments.first()) return message.inlineReply(new Discord.MessageEmbed()
.setColor('#eb5c0e')
...
if(message.attachments.first()) {
  try {
    ...
    return message.guild.emojis.create(message.attachments.first().url, message.attachments.first().name.split('.')[0], { reason: `Sorumlu moderatör: `+message.author.tag }).then(emoji => message.inlineReply(`Emoji yüklendi: ${message.guild.emojis.cache.get(emoji.id)}`))

  } catch(error) {
    console.log(error);
    console.log('başarışız');
  ...
  if(args.slice(0).join(' ').startsWith('https://') && args.slice(0).join(' ').includes('discord')) {
    try {
      ...
      return message.guild.emojis.create(args.slice(0).join(' '), args.slice(0).join(' ').split('/')[4].split('.')[0], { reason: `Sorumlu moderatör: `+message.author.tag }).then(emoji => message.inlineReply(`Emoji yüklendi: ${message.guild.emojis.cache.get(emoji.id)}`)).catch(err => message.inlineReply(`Bir hata oluştu. Lütfen; 
! Sunucuda emoji yüklemek için yer olduğuna, 
! Koyduğunuz dosyanın bir fotoğraf/gif olduğuna, 
! 256kb boyutundan küçük olduğuna,
emin olun ve tekrar deneyin.`))
...
const s = args.slice(0).join(' ').split(' ').filter(x => x.includes('<') && x.includes('>'))[0];
  try {
    ...
    return message.guild.emojis.create(url, isim, { reason: `Sorumlu moderatör: `+message.author.tag }).then(emoji => message.inlineReply(`Emoji yüklendi: ${message.guild.emojis.cache.get(emoji.id)}`));
