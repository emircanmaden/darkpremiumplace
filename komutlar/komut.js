...
if(!message.member.permissions.has('ADMINISTRATOR')) return message.inlineReply(new Discord.MessageEmbed().setThumbnail(message.author.avatarURL() ? message.author.avatarURL({dynamic: true}) : '').setTitle('Bir hata oldu!').setDescription(`• \`${client.ayarlar.prefix}komut\` **kullanmak için,** \`Yönetici\` **yetkisine sahip olman gerekiyor.**`));
if(!args[0]) return message.inlineReply(errorEmbed.setDescription(`\`${client.ayarlar.prefix}komut ekle\`
\`${client.ayarlar.prefix}komut sil\`
\`${client.ayarlar.prefix}komut tablo\` **yazarak kullanabilirsin.**`));

if(args[0] === 'ekle') {
if(!args[1]) return message.inlineReply(errorEmbed.setDescription(`${message.author} **Örnek kullanım:**
\`\`\`${client.ayarlar.prefix}komut ekle tag 🚀\`\`\``));

if(!args[2]) return message.inlineReply(errorEmbed.setDescription(`${message.author} **Örnek kullanım:**
\`\`\`${client.ayarlar.prefix}komut ekle tag 🚀\`\`\``));

data.push(`chimped.${message.guild.id}`, { command: args[1], respond: args.slice(2).join(' ') });
message.inlineReply(errorEmbed.setTitle('İşte bu kadar!')
.setDescription(`**Komut:** \`${args[1]}\`\n**Komut Cevabı:** \`${args.slice(2).join(' ')}\` **başarıyla kuruldu.**`)
.setThumbnail('')
.setFooter(`Sistemi aktif eden: ${message.author.username}`, message.author.avatarURL() ? message.author.avatarURL({dynamic: true}) : ''));
};

if(args[0] === 'tablo') {
const chimped = await data.fetch(`chimped.${message.guild.id}`);
if(!chimped) return message.inlineReply(new Discord.MessageEmbed()
.setTitle(`Tablo'da herhangi bir komut yok!`));;
var i = 1;
message.inlineReply(errorEmbed.setColor('#00001')
...
if(!chimped.find(a => a.command === args[1])) return message.inlineReply(new Discord.MessageEmbed()
.setTitle(`**\`${args[1]}\` Yazdığın komut tablo'da yok!
